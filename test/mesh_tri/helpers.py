...
def assert_mesh_consistency(mesh):
    assert numpy.all(numpy.logical_xor(mesh.is_boundary_facet, mesh.is_interior_facet))
    ...
    assert numpy.all(mesh.is_boundary_facet[mesh.edges_cells["boundary"][0]])
    assert not numpy.any(mesh.is_boundary_facet[mesh.edges_cells["interior"][0]])

    for facet_id, cell_id, local_facet_id in mesh.edges_cells["boundary"].T:
        assert facet_id == mesh.cells["facets"][cell_id][local_facet_id]
    ...
    for (
        ...
        local_facet_id0,
        local_facet_id1,
    ) in mesh.edges_cells["interior"].T:
        assert facet_id == mesh.cells["facets"][cell_id0][local_facet_id0]
        assert facet_id == mesh.cells["facets"][cell_id1][local_facet_id1]

    # check consistency of edges_cells_idx with edges_cells
    for facet_id, idx in enumerate(mesh.edges_cells_idx):
        if mesh.is_boundary_facet[facet_id]:
            assert facet_id == mesh.edges_cells["boundary"][0, idx]
        else:
            assert facet_id == mesh.edges_cells["interior"][0, idx]

    # Assert edges_cells integrity
    for cell_gid, facet_gids in enumerate(mesh.cells["facets"]):
        for facet_gid in facet_gids:
            idx = mesh.edges_cells_idx[facet_gid]
            if mesh.is_boundary_facet[facet_gid]:
                assert cell_gid == mesh.edges_cells["boundary"][1, idx]
            else:
                assert cell_gid in mesh.edges_cells["interior"][1:3, idx]

    # make sure the facets are opposite of the points
    for cell_gid, (point_ids, facet_ids) in enumerate(
        zip(mesh.cells["points"], mesh.cells["facets"])
    ):
        for k in range(len(point_ids)):
            assert set(point_ids) == set(
                [*mesh.facets["points"][facet_ids][k], point_ids[k]]
            )

    # make sure the is_boundary_point/facet/cell is consistent
    ref_cells = numpy.any(mesh.is_boundary_facet_local, axis=0)
...
def assert_mesh_equality(mesh0, mesh1):
    ...
    assert numpy.all(numpy.abs(mesh0.points - mesh1.points) < 1.0e-14)
    assert numpy.all(mesh0.facets["points"] == mesh1.facets["points"])

    # # Assume that in mesh1, the rows are ordered such that the facet indices [0] are in
    ...
    # # opposed to boundary facets, there can be no new interior facets, just some are
    # # removed.)
    ...
    assert numpy.all(mesh0.boundary_facets == mesh1.boundary_facets)
    assert numpy.all(mesh0.interior_facets == mesh1.interior_facets)
    ...
    assert numpy.all(mesh0.is_boundary_facet_local == mesh1.is_boundary_facet_local)
    assert numpy.all(mesh0.is_boundary_facet == mesh1.is_boundary_facet)
